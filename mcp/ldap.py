...
class HackspaceIPA():
    ...
    def create_ipa_user(self, username, name, email):
        ename = name.split()
        tmp_pass = ''.join(random.choice(string.ascii_lowercase + string.ascii_uppercase + string.digits)
                           for _ in range(20))
        return self.ipa.user_add(username, opts={'givenname': ename[0],
                                                 'sn': ename[-1],
                                                 'cn': name,
                                                 'displayname': username,
                                                 'mail': email,
                                                 'userpassword': tmp_pass})

    def change_ipa_user_password(self, username, new_password, **kwargs):
        ...
        r = requests.post('https://'+os.environ.get('IPA_URL')+'/ipa/session/change_password',
                          data=params,
                          headers=headers)
