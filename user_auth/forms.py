...
class SignupForm(forms.Form):
    first_name = forms.CharField(label="First ame",
                                 max_length=100,
                                 required=True,
    ...
    password_check = forms.CharField(label="Re-enter Password",
                                     min_length=8,
                                     widget=forms.PasswordInput(attrs={"placeholder": "Re-enter password"}))

    username = ldap.LDAPUsernameField(label="Username",
                                      max_length=100,
                                      widget=forms.TextInput(attrs={"placeholder": "Username"}),
                                      help_text="You can use this to login and we will use it to refer to you in"
                                                "internal systems")
    ...
    emergency_contact_name = forms.CharField(label="Emergency Contact Name",
                                             ...
                                             help_text=mark_safe("Please note that we will, obviously, contact"
                                                                 "emergency services, use this field to let us know who"
                                                                 "we should inform about it."))

    emergency_contact_num = forms.CharField(label="Emergency Contact Phone Number",
                                            widget=forms.TextInput(attrs={"placeholder": "Emergency Contact Number"}),
